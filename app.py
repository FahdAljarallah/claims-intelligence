...
# محرك استخراج ديناميكي خالص (Pure Dynamic Parser - Zero Hardcoding)
def parse_claims_pure_dynamic(file_bytes, file_name, session_id, default_members):
    cleaned_records = []
...
def process_preview_files(uploaded_files, session_id, default_members):
    ...
    for f in uploaded_files:
        if f.name.lower().endswith('.pdf'):
            file_bytes = f.read()
            m_recs = parse_claims_pure_dynamic(file_bytes, f.name, session_id, default_members)
            all_m.extend(m_recs)
...
st.title("مرصد المطالبات | الاستخراج الديناميكي الخالص")
st.markdown("تحليل واستخراج البيانات آلياً من ملفات الـ PDF دون أي قيم مسبقة أو مكتوبة يدوياً.")
...
if uploaded_files:
    ...
    if st.button("تشغيل الاستخراج الديناميكي", type="secondary"):
        if not current_premium or not total_members or not inception_date:
            st.warning("يرجى تعبئة الحقول الأساسية.")
        else:
            with st.spinner("جاري استخراج وتحليل بيانات الملفات ديناميكياً..."):
                ...
                st.success(f"تمت المعالجة الديناميكية لـ {len(unique_files)} ملفات بنجاح (`{', '.join(unique_files)}`) بإجمالي {total_records} سجلاً مستخلاصاً!")

    if "preview_m" in st.session_state and not st.session_state["preview_m"].empty:
        st.subheader("🔍 معاينة جدول الأداء الديناميكي (Pure Dynamic Preview)")
        ...
        if st.button("اعتماد وضخ البيانات الديناميكية إلى BigQuery", type="primary"):
            with st.spinner("جاري الضخ إلى المستودع..."):
                ...
                st.success("تم ضخ البيانات ديناميكياً بنجاح إلى BigQuery وجاهزة للتحليل المالي التنفيذي!")
