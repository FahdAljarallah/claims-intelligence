...
# محرك موحد يلتقط جداول الأداء الشهري للتعاونية وميدغلف بمعايير مرنة
def parse_pdf_claims_unified(file_obj, session_id, default_members):
    file_obj.seek(0)
        ...
        with pdfplumber.open(file_obj) as pdf:
            ...
            for page in pdf.pages:
                ...
                while i < len(lines):
                    ...
                    # التقاط أنماط الأشهر بمرونة تامة (MM/YYYY أو YYYY-MM أو حتى أسماء الأشهر الرقمية)
                    ...
                    if date_match:
                        ...
                        # تجميع سياق الأرقام المجاورة للسطر لضمان التقاط الأرصدة والمطالبات في ميدغلف والتعاونية
                        context_numbers = []
...
def process_preview_files(uploaded_files, session_id, default_members):
    ...
    for f in uploaded_files:
        if f.name.lower().endswith('.pdf'):
            f.seek(0)
            m_recs = parse_pdf_claims_unified(f, session_id, default_members)
            all_m.extend(m_recs)
...
st.title("مرصد المطالبات | المحرك الموحد الشامل")
st.markdown("استخراج دقيق ومكتمل لبيانات التعاونية وميدغلف معاً دون أي نقصان.")
...
if uploaded_files:
    ...
    if st.button("معاينة واستخراج كافة الملفات", type="secondary"):
        if not current_premium or not total_members or not inception_date:
            st.warning("يرجى تعبئة الحقول الأساسية.")
        else:
            with st.spinner("جاري استخراج ودمج الملفات بالكامل..."):
                ...
                st.success(f"تمت بنجاح معالجة {len(unique_files)} ملفات (`{', '.join(unique_files)}`) بإجمالي {len(df_m)} سجلاً كاملاً!")

    if "preview_m" in st.session_state and not st.session_state["preview_m"].empty:
        st.subheader("🔍 معاينة جدول الأداء الموحد الشامل (Unified Preview)")
        ...
        st.download_button(
            ...
            data=csv_m,
            file_name="unified_suppliers_performance.csv",
            mime="text/csv",
        )
        
        if st.button("اعتماد وضخ البيانات المكتملة إلى BigQuery", type="primary"):
